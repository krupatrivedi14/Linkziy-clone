      ...
      <div className={styles.authWrap}>
        ...
        <div className={styles.formPane}>
          <div className={styles.formCard}>
            ...
            <h2 className={styles.formHeading}>Let's get started</h2>
            <p className={styles.formSub}>Set up your profile and start your growth workflow in minutes.</p>

            <form className={styles.form} onSubmit={handleSubmit} noValidate>
              <div className={styles.fieldRow}>
                <div className={styles.field}>
                  <label htmlFor="su-name" className={styles.label}>Full Name</label>
                  <input id="su-name" type="text" placeholder="John Smith"
                    className={`${styles.input} ${errors.name ? styles.inputErr : ''}`}
                    value={form.name} onChange={set('name')} />
                  {errors.name && <span className={styles.errMsg}>{errors.name}</span>}
                </div>
                <div className={styles.field}>
                  <label htmlFor="su-company" className={styles.label}>Company <span className={styles.optional}>(optional)</span></label>
                  <input id="su-company" type="text" placeholder="Acme Corp"
              ...
              <div className={styles.field}>
                ...
                <input id="su-email" type="email" placeholder="you@company.com"
                  ...
                  value={form.email} onChange={set('email')} autoComplete="username" />
                {errors.email && <span className={styles.errMsg}>{errors.email}</span>}
              </div>

              <div className={styles.field}>
                <label htmlFor="su-pass" className={styles.label}>Password</label>
              ...
              <label className={`${styles.checkLabel} ${styles.termsLabel}`}>
                <input type="checkbox" checked={terms} onChange={e => setTerms(e.target.checked)} className={styles.checkInput} />
                <span className={styles.checkBox}>{terms && <svg width="10" height="10" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="1 6 4.5 9.5 11 2.5"/></svg>}</span>
                I agree to the <button type="button" className={styles.switchLink}>Terms and Conditions</button>
              </label>
              {errors.terms && <span className={styles.errMsg}>{errors.terms}</span>}
            ...
            <p className={styles.switchText}>
              ...
              <button className={styles.switchLink} onClick={() => onNavigate && onNavigate('signin')}>Sign in</button>
            </p>
          </div>
        </div>
